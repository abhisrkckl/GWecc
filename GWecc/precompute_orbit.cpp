 ...
 */
int orbital_evolution_ode_rhs(double tau, const double y[], double f[], void *params) {
...
OrbitIntegrals precompute_orbit(const double emin, const double taumax) {
    ...
    gsl_odeiv2_system sys = {orbital_evolution_ode_rhs, NULL, neqs, NULL};
    ...
    while(tau<taumax){
        ...
        if(status != GSL_SUCCESS) {
            throw std::runtime_error("[" __FILE__ "] Solving ODE to pre-compute tau(e) failed.");
        }
