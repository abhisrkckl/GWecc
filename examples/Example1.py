...
def PN_periastron_advance(M, q, n, e):
    ...
    return (
        3 * xi / ipow(OTS, 2)
        + ((78 + ipow(e, 2) * (51 - 26 * eta) - 28 * eta) * ipow(xi, 2))
        / (4.0 * ipow(OTS, 4))
        + (
            (
                18240
                - 25376 * eta
                + 896 * ipow(eta, 2)
                + ipow(e, 4) * (2496 - 1760 * eta + 1040 * ipow(eta, 2))
                + (1920 + ipow(e, 2) * (3840 - 1536 * eta) - 768 * eta) * OTS
                + 492 * eta * ipow(M_PI, 2)
                + ipow(e, 2)
                * (
                    28128
...
plt.text(
    ...
    ha="center",
    va="center",
    bbox=dict(boxstyle="round", facecolor="cyan", alpha=0.1),
)
