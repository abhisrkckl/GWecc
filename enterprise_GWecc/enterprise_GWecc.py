...
def eccentric_cw_delay(
    ...
    if (log10_dist is None and log10_h is None) or (
        log10_dist is not None and log10_h is not None
    ):
        raise ValueError(
            "One and only one of log10_dist and log10_h should be used. The other should be set to None."
        )
    elif log10_dist is not None:
        D_GW = 1e6 * (10.0**log10_dist)
    else:
        H0 = 10.0**log10_h
        x0 = (M * n0) ** (2 / 3)
        eta = q / (1 + q) ** 2
